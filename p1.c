...
#define SERVER_prev "10.254.223.61"
#define SERVER_me   "10.254.223.60"
#define SERVER_next "10.254.223.62"
...
int main(int argc, char **argv)
{
    ...
    //*******************
    //TRECO DE CODIGO EXCLUSIVO DO PLAYER 1
    //*******************
    ...
    data->msg_type = POSITIONING_PHASE;

    pack_msg (message, data, 2, 1, 0x00);
    ...
    pack_msg (message, data, 3, 1, 0x00);
    sendto (sock, message, MSG_SIZE, 0, (struct sockaddr *) &ring_next, next_len);


    recvfrom(sock, message, MSG_SIZE, 0, (struct sockaddr *) &ring_previous, &prev_len);
    pack_msg (message, data, 4, 1, 0x00);
    sendto(sock, message, MSG_SIZE, 0, (struct sockaddr *) &ring_next, next_len);
